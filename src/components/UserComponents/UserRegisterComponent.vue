    ...
    <form v-on:submit.prevent="submit" class="bg-white w-full sm:w-1/2 h-auto rounded-lg shadow-2xl">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 p-10 gap-4">
        <div class="divin" :class="{'text-red-600': $v.document.$error}">
          ...
          <input class="inputs" :class="{ 'postin': $v.document.$error }" v-model.trim="$v.document.$model"
        ...
        <div class="divin" :class="{'text-red-600': $v.names.$error}">
          ...
          <input class="inputs" :class="{ 'postin': $v.names.$error }" v-model.trim="$v.names.$model" id="names"
                 type="text">
        </div>
        <div class="divin" :class="{'text-red-600': $v.last_names.$error}">
          ...
          <input class="inputs" :class="{ 'postin': $v.last_names.$error }" v-model.trim="$v.last_names.$model"
                 id="lnames" type="text">
        </div>
        <div class="divin" :class="{'text-red-600': $v.direcction.$error}">
          <label for="dir">Direccion.</label>
          <p v-if="!$v.direcction.maxLength">Direccion exede lo permitido</p>
          <input class="inputs" :class="{ 'postin': $v.direcction.$error }" v-model.trim="$v.direcction.$model" id="dir"
                 type="text">
        </div>
        <div class="divin" :class="{'text-red-600': $v.tellephone1.$error}">
          ...
          <input class="inputs" :class="{ 'postin': $v.tellephone1.$error }" v-model.trim="$v.tellephone1.$model"
                 id="tell1" type="number">
        </div>
        <div class="divin" :class="{'text-red-600': $v.tellephone2.$error}">
          ...
          <input class="inputs" :class="{ 'postin': $v.tellephone2.$error }" v-model.trim="$v.tellephone2.$model"
                 id="tell2" type="number">
        </div>
        <div class="divin" :class="{'text-red-600': $v.mail.$error}">
          ...
          <input class="inputs" :class="{ 'postin': $v.mail.$error }" v-model.trim="$v.mail.$model" id="mail"
                 type="email">
        </div>
        <div class="divin" :class="{'text-red-600': $v.password2.$error}">
          <label for="contra1">Contraseña</label>
          <p v-if="!$v.password2.minLength">Contraseña muy corto</p>
          <input class="inputs" :class="{ 'postin': $v.password2.$error }" v-model.trim="$v.password2.$model"
                 id="contra1" type="password">
        </div>
        <div class="divin" :class="{'text-red-600': $v.password.$error}">
          ...
          <input class="inputs" :class="{ 'postin': $v.password.$error }" v-model.trim="$v.password.$model" id="contra2"
                 type="password">
        </div>
        <div class="divin" :class="{'text-red-600': $v.depar.$error}">
          <label for="departament">Departamento</label>
          <select class="inputs" :class="{ 'postin': $v.depar.$error }" v-model.trim="$v.depar.$model" v-on:click="muni"
                  id="departament">
            <option disabled value="">Seleccione un elemento</option>
        ...
        <div class="divin" :class="{'text-red-600': $v.fk_municipality_id.$error}">
          ...
          <select class="inputs" :class="{ 'postin': $v.fk_municipality_id.$error }"
                  v-model.trim="$v.fk_municipality_id.$model" name="muni" id="muni">
            <option disabled value="">Seleccione un elemento</option>
            <option v-for="(item, index) in allMunicipalities" :key="index"
                    v-bind:value="{fk_municipality_id: item.id}">
              {{ item.name }}
            </option>
          </select>
